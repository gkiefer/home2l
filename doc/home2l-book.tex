...
\item
  ...
  \infobox{
    ...
    \lst{home2l demo} script (see Section~\ref{sec:tutorial-mqtt-imports}).
  }
...
Sometimes, a user may not want or need a cluster installtion, but only use individual tools and drivers on a single machine and integrate them into an existing environment. In such a case, the installation and administration is \textit{much} simpler than this chapter may suggest. Please skip over to Section~\ref{sec:managing-standalone} for information on a standalone installation.
...
A typical \textit{Home2L} installation is distributed over multiple computers (\textit{machines}), on each of which one ore multiple \textit{Home2l instances} may be running. There is no central server, all \textit{Home2L instances} are equal in rank.

This chapter assumes a cluster installation with multiple \textit{Home2L} instances potentially running on multiple machines. For a standalone use of selected tools without networking, see Section~\ref{sec:managing-standalone}.
...
\label{sec:managing-standalone}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


Sometimes, a user may not want to exploit the full power of the \textit{Resources} library and just wants to use individual tools and drivers and integrate them into an existing environment. In such a case, the installation is much simpler.

The following instructions describe a standalone installation assuming that only the driver \reftool{home2l-drv-brownies} is to be used, and that some of its sensors are to be exported via MQTT (\reftool{home2l-drv-mqtt}).
...
\begin{itemize}
  \item \lst{0}, \lst{-}, \lst{false}, \lst{off}, \lst{no}
  \item \lst{1}, \lst{+}, \lst{true}, \lst{on}, \lst{yes}
\end{itemize}
...
\begin{itemize}
  ...
  \item There is a hierarchical name space for identifying resources (\textit{Home2L} URIs and MQTT topics, respectively). The syntax is very similar.
\end{itemize}
...
Messages to the state topic are reported as valid values for the respective resource. If a message for the
validity indicates that the client is offline, a state of \refapic{rcsUnknown} is reported.
