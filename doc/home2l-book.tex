...
The following subsections give an overview on the various triggered functions (Section~\ref{sec:rules-events}), on accessing resource values and states (Section~\ref{sec:rules-values}), and on placing requests (Section~\ref{sec:rules-requests}).  Details can be found in the \theapipython{} documentation, which serves as a reference manual for rules writing.
...
Gates are new read-only resources continuously showing values that are calculated by their \textit{gate function} based on values of other (source) resources. They can be seen as (logical) components in a circuit of automation rules.
