...
\textit{Home2L} automation rules are normal Python scripts. They can be tested separately from already existing rules files and later be merged into them. Section~\ref{sec:tutorial-rules-example} describes this process for an example. Section~\ref{sec:tutorial-rules-python} gives an in-depth introduction to the \theapipython{} by means of an interactive Python session. Finally, the supplied \reftool{rules-showhouse} file will be inspected and extended in Section~\ref{sec:tutorial-rules-showhouse}.
...
In this section, we will deactivate the provided \reftool{rules-showhouse} script and write an own rules file to control the front light of the show house. The \textit{Home2L} Python package offers a number of different techniques to implement automation rules (see Chapter~\ref{ch:rules}). In this section, some of them are demonstrated and sometimes compared to alternative solutions for the same problem.
...
The supplied automation script \reftool{rules-showhouse} (source tree: \refsrc{showcase/etc/rules-showhouse}) contains a couple of example rules, the effects of which you may have already observed in previous tutorial steps. These are rules for
\begin{itemize}
...
\item
  ...
  (source tree: \refsrc{showcase/etc/rules-showhouse}):
  \begin{lstlisting}[language=bash]
...
\item
  ...
  (source tree: \refsrc{showcase/home2l-showhouse}):
  \begin{lstlisting}[language=bash]
...
\item
  Inspect and read the main configuration file (source tree: \refsrc{showcase/etc/home2l.conf}):
  \begin{lstlisting}[language=bash]
...
Cross-compilation is supported by the \textit{Home2L} build system based on the Debian cross-building capabilities for the architectures \textit{amd64} and \textit{armhf}. The development machine must have \textit{amd64} set as its primary architecture. The other architecture(s) must be entered as additional architectures to the \textit{dpkg(1)} package manager.
...
To use the \textit{Remind} backend, the \textit{remind(1)} command line tool
must be installed on some computer reachable by \textit{ssh(1)} by the \textit{WallClock}.


\subsection{The \textit{iCal} Backend}
\labeltool{home2l-pimd}

The \textit{iCal} backend supports calendars stored as \textit{iCalendar} files
(\textit{.ics}) in the \href{https://vdirsyncer.pimutils.org/en/stable/vdir.html}{Vdir storage format}.
The external tool \href{https://vdirsyncer.pimutils.org/}{vdirsyncer} can then be used to
synchronize the calendar with external \textit{CalDAV} repositories.

The storage directory is maintained by the \textit{Home2L} PIM daemon \reftool{home2l-pimd},
which must be running on some host (typically as a daemon controlled by \reftool{home2l-daemon}).
The \textit{WallClock} instances communicate with the PIM daemon.

\reftool{home2l-pimd} automatically executes \textit{vdirsyncer} in the background.

On the host running \reftool{home2l-pimd}, \textit{vdirsyncer(1)} and \textit{nc(1)},
the OpenBSD variant of \textit{netcat}, must be installed. The host must be reachable by
\textit{ssh(1)} by the \textit{WallClock}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The \textit{Fronius} Driver}
\label{sec:drvlib-fronius} \labeltool{home2l-drv-fronius}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\subsection{Description}
\label{sec:drvlib-fronius-description}

The \textit{Fronius} driver exports data provided by
\textit{Fronius Datamanager 2.0} interfaces typically found in
solar inverters of the named manufacturer.


\subsection{Configuration Parameters}
\label{sec:drvlib-fronius-env}
\input{ref_env_drv_fronius}


\subsection{Provided Resources}
\label{sec:drvlib-fronius-rc}

The driver is capable of providing any \textit{inverter} or \textit{meter} real-time
data as delivered by the \textit{Fronius Solar API V1}. By default, the solar power
(\lst{inverter/PAC}) and the real power value reported by the primary smart meter
(\lst{meter/PowerReal_P_Sum}) are provided. Other values can be provided by
editing \refsrc{drivers/fronius/home2l-drv-fronius}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The \textit{Fingerprint} Driver}
\label{sec:drvlib-fingerprint}
\labeltool{home2l-drv-fingerprint} \labeltool{home2l-fingerprint2l}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\subsection{Description}
\label{sec:drvlib-fingerprint-description}

The \textit{fingerprint} driver supports matching and enrollment using fingerprint sensonrs. Presently, sensors of type \textit{Grow R503} are supported.

For installation and testing, \reftool{home2l-fingerprint2l} provides some diagnostic operation.
Run \lstf{home2l-fingerprint2l -h} to get details.

In \textbf{matching mode}, the sensor is monitored continuously, and the presence of a known,
identified finger is reported by \refrc{fingerprint/match} showing a valid value $\ge 0$.
The aura LED is only lit if a known finger has been detected. In all other cases (i.e. if an
unknown finger is presented), the LED stays off.

In \textbf{enrollment mode}, new fingerprints are enrolled and stored in free template locations
of the sensor's internal non-volatile memory. To enroll a finger, the user has to touch and release
the sensor multiple times (see \refenv{fingerprint.samples}). The process is guided by the aura
LED showing on of the following:

\begin{itemize}
  \item Breathing light, white or purple: Ready for enrolling a new finger.
  \item Solid blue: Successfully captured a sample -- lift the finger and touch again.
  \item Solid red: Faild to capture a sample -- lift the finger and touch again.
  \item Fast flashing light, white or purple: Waiting for the next sample of the same finger.
  \item Flashing blue (7 times): Successfully created template -- well done!
  \item Flashing red (5 times): Failed to create a template from your samples -- please try again.
\end{itemize}

A note on security: The user is responsible for security and must be aware that various attacks
are possible if physical access to the sensor or the serial link are possible. It is essential
that a) the sensor cannot be easily dismounted and b) the communication cable cannot be accessed
easily.


\subsection{Prerequisites}
\label{sec:drvlib-fingerprint-prerequisites}

For the driver to operate, the Python module \lst{python-serial} is required,
the installation of \lst{setserial} is recommended.


\subsection{Configuration Parameters}
\label{sec:drvlib-fingerprint-env}
\input{ref_env_drv_fingerprint}


\subsection{Provided Resources}
\label{sec:drvlib-fingerprint-rc}
\input{ref_rc_drv_fingerprint}
